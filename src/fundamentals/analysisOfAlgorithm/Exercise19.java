 ...
 * @package fundamentals.analysisOfAlgorithm
 * @date 1/31/21
 * @comment: Local minimum of a matrix. Given an N-by-N array a[] of N 2 distinct integers, design
 * an algorithm that runs in time proportional to N to find a local minimum: a pair of indices i and
 * j such that a[i][j] < a[i+1][j], a[i][j] < a[i][j+1], a[i][j] < a[i-1][j], and a[i][j] <
 * a[i][j-1]. The running time of your program should be proportional to N in the worst case.
 */
public class Exercise19 {
  ...
  public static void main(String[] args) {
    ...
    StdOut.println(
        "Local Minimum Matrix: " + localMinimum3 + " Expected: 1"); //2 and 4 would also be valid
  ...
  //O(n)
  private static int localMinimumMatrix(int[][] matrix, int firstRow, int endRow, int firstColumn,
      int endColumn) {
    ...
    if (currentMinimumElementRow - 1 >= 0
        && matrix[currentMinimumElementRow][currentMinimumElementColumn] > matrix[
        currentMinimumElementRow - 1][currentMinimumElementColumn]) {
      ...
      return localMinimumMatrix(matrix, firstRow, endRow, firstColumn, endColumn);
        ...
        && matrix[currentMinimumElementRow][currentMinimumElementColumn]
